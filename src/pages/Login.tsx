      ...
      <div className="relative z-10 w-full max-w-md p-4">
        ...
        <div className="text-center mb-10 animate-reveal">
          <div className="flex items-center justify-center gap-4 mb-6">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-primary shadow-[0_0_20px_rgba(var(--primary),0.3)] text-glow transition-transform hover:scale-110 duration-500">
        ...
        <Card className="glass animate-reveal delay-100 overflow-hidden border-none ring-1 ring-white/10">
          <CardHeader className="p-8 pb-0">
            <Tabs defaultValue="login" className="w-full">
              <TabsList className="grid w-full grid-cols-2 mb-8 bg-muted/20 p-1 rounded-xl">
                <TabsTrigger value="login" className="text-sm font-bold py-2.5 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-lg">Login</TabsTrigger>
                <TabsTrigger value="register" className="text-sm font-bold py-2.5 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-lg">Register</TabsTrigger>
              </TabsList>

              <div className="space-y-4 mb-8">
                <p className="text-[11px] uppercase tracking-widest font-bold text-center text-muted-foreground/80">Identify your access role</p>
                <div className="grid grid-cols-3 gap-3">
                  {(Object.entries(roleConfig) as [AppRole, typeof roleConfig[AppRole]][]).map(([role, config], idx) => (
                    <button
                      key={role}
                      onClick={() => setSelectedRole(role)}
                      className={`flex flex-col items-center gap-3 p-4 rounded-xl border transition-all duration-300 animate-reveal ${selectedRole === role
              ...
              <TabsContent value="login" className="animate-reveal delay-400">
                <LoginForm
                  role={selectedRole}
                ...
                <div className="relative my-8">
                  ...
                  <div className="relative flex justify-center text-[10px] uppercase tracking-widest font-bold"><span className="bg-transparent px-3 text-muted-foreground">Demo Access</span></div>
                </div>
              ...
              <TabsContent value="register" className="animate-reveal delay-400">
                <RegisterForm
                  role={selectedRole}
                  onSuccess={handleAuthSuccess}
                  onError={(msg) => toast.error(msg)}
        ...
        {/* Footer info */}
        <div className="mt-8 text-center animate-reveal delay-500">
          <p className="text-[10px] text-muted-foreground/40 font-medium">Volvitech &copy; 2026. All rights secured.</p>
        </div>
      </div>
    </div>
  );
